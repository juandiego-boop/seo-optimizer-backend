...
router.get('/auth/callback',async(req,res)=>{
  ...
  try{
    const t=await axios.post(`https://${shop}/admin/oauth/access_token`,{client_id:process.env.SHOPIFY_API_KEY,client_secret:process.env.SHOPIFY_API_SECRET,code});
    const r=await pool.query('INSERT INTO shops(shop_domain,access_token,credits)VALUES($1,$2,50) ON CONFLICT(shop_domain)DO UPDATE SET access_token=$2 RETURNING *',[shop,t.data.access_token]);
